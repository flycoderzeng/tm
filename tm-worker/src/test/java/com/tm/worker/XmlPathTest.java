...
public class XmlPathTest {
    public static void main(String[] args) throws DocumentException {
        ...
        String s = XMLUtils.formatXml(xml);
        System.out.println(s);
        String xpath = "//dependency/groupId/text()";
        Document doc = XMLUtils.parseXmlString(s);
        List<Node> list = XMLUtils.selectNodeList(doc, xpath);
        for (Node node : list) {
            System.out.println(node.getStringValue());
        }

        xpath = "//dependency/@id";
        list = XMLUtils.selectNodeList(doc, xpath);
        for (Node node : list) {
            System.out.println(node.getStringValue());
        }
